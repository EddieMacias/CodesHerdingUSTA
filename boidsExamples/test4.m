%% optimizacion de distancias ejemplo de fabrica
tic
...
for t = 1:num_iterations
    ...
    initialGuess.x = shepherds(1);
    initialGuess.y = shepherds(2); 
    X = positions(:,1)';
    Y = positions(:,2)';
    ...
    rx = min(X);
    ry = min(Y);
    problem.Constraints.dmin = d >= 0.2;
    problem.Constraints.cx0   = x <= rx - 0.5;
    problem.Constraints.cx1  = x <= -1;

    [sol,optval] = solve(problem,initialGuess);
    target_L = [sol.x sol.y];
    ...
    scatter(target_L(1), target_L(2),'cyan')
