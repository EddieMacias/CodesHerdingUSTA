...
function [R, posicion_recompensa] = GenerarMatrizR(posiciones_castigos,posicion_recompensa,num_filas,num_estados, num_acciones,pos_Agent)
    ...
    %evitar recompensas Secuestradas
    for pos=1:length(R)
        if (R(pos) == -100)
            if pos == posicion_recompensa
                posicion_recompensa = pos_Agent;
            end
        end
    end
