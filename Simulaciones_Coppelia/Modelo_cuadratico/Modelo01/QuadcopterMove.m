       ...
       for t = 1:num_iterations
           ...
           % conexion Enjambre 01
            ...
            trajectory10 (t,:) = [shepherds(4,:), boidAlt];
                ...
                % target_R = [positions(pos(1),1)+ 0.5, positions(pos(1),2)];
                % pos = find(positions(:,1)==L_pos);
                % target_L = [positions(pos(1),1)- 0.5, positions(pos(1),2)];
                % pos = find(positions(:,2)==U_pos);         
                % target_U = [positions(pos(1),1), positions(pos(1),2)+0.5];
                % pos = find(positions(:,2)==D_pos);         
                % target_D = [positions(pos(1),1), positions(pos(1),2)-0.5];
                % Target = [target_R ; target_L; target_U ;target_D];  
                [target_R, target_L, target_D, target_U] = getTarget(positions, [0,0]);
