...
errpercent = zeros(1,5);
errpercent(1) = err(1)*100/300;
errpercent(2) = err(2)*100/300;
errpercent(3) = err(3)*100/300;
errpercent(4) = err(4)*100/300;
errpercent(5) = err(5)*100/300;
accError = mean(errpercent);
...
xlabel('Ovejas');
ylabel('Errores');
msg = sprintf('Error Acumulado del metodo Boids + Optim + MARL : %f ',accError);
title(msg);
grid on;
grid minor
ylim([0 300]);
