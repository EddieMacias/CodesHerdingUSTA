    ...
    if (res1  == sim.simx_return_ok && ...
       ...
       for t = 1:num_iterations
           ...
           % conexion Enjambre 01
            ...
            % -------------------------------------------------------------------------------------------------------
            % Calculo de Referencias Targets
            shepherds_refs = [positions; leader];
            target_R = setOptimTarget(shepherds_refs,'R',shepherds(4,:),0.1);
            target_L = setOptimTarget(shepherds_refs,'L',shepherds(2,:),0.1);
            target_U = setOptimTarget(shepherds_refs,'U',shepherds(1,:),0.1);
            target_D = setOptimTarget(shepherds_refs,'D',shepherds(3,:),0.1);
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    [res6] = sim.simxSetObjectPosition(clientID, target_handle7, -1, coord, sim.simx_opmode_blocking);
                    shepherds(1,:) = xycoord;
                end
            end
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res7] = sim.simxSetObjectPosition(clientID, target_handle8, -1, coord, sim.simx_opmode_blocking);
                    shepherds(2,:) = xycoord;
                end
            end
           ...
           [R_D, max_recompensa_D]  = GenerarMatrizR(posiciones_castigos_D,...
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res8] = sim.simxSetObjectPosition(clientID, target_handle9, -1, coord, sim.simx_opmode_blocking);
                    shepherds(3,:) = xycoord;
                end
            end
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res9] = sim.simxSetObjectPosition(clientID, target_handle10, -1, coord, sim.simx_opmode_blocking);
                    shepherds(4,:) = xycoord;
                end
            end
