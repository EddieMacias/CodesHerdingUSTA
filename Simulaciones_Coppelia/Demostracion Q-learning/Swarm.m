...
if (clientID > -1)
    ...
    [res11, target_handle11] = sim.simxGetObjectHandle(clientID, 'TestTarget',          sim.simx_opmode_blocking);
    [res12, target_handle12] = sim.simxGetObjectHandle(clientID, 'TestTarget0',          sim.simx_opmode_blocking);
    [res13, target_handle13] = sim.simxGetObjectHandle(clientID, 'TestTarget1',          sim.simx_opmode_blocking);
    [res14, target_handle14] = sim.simxGetObjectHandle(clientID, 'TestTarget2',          sim.simx_opmode_blocking);
    ...
    if (res1  == sim.simx_return_ok && ...
        ...
        res11 == sim.simx_return_ok && ...
        res12 == sim.simx_return_ok && ...
        res13 == sim.simx_return_ok && ...
        res14 == sim.simx_return_ok)
       ...
       for t = 1:num_iterations
           ...
           % conexion Enjambre 01
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res7] = sim.simxSetObjectPosition(clientID, target_handle7, -1, coord, sim.simx_opmode_blocking);
                    shepherds(1,:) = xycoord;
                end
            end
            ...
            for acciones=1:cnt 
                ...
                xycoord = SearchIndexOnTable(Q_board,accion);
                coord = [xycoord boidAlt];
                [res12] = sim.simxSetObjectPosition(clientID, target_handle12, -1, coord, sim.simx_opmode_blocking);
            end
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res8] = sim.simxSetObjectPosition(clientID, target_handle8, -1, coord, sim.simx_opmode_blocking);
                    shepherds(2,:) = xycoord;
                end
            end
           ...
           [R_D, max_recompensa_D]  = GenerarMatrizR(posiciones_castigos_D,...
            ...
            for acciones=1:cnt 
                ...
                xycoord = SearchIndexOnTable(Q_board,accion);
                coord = [xycoord boidAlt];
                [res13] = sim.simxSetObjectPosition(clientID, target_handle13, -1, coord, sim.simx_opmode_blocking);
            end
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res9] = sim.simxSetObjectPosition(clientID, target_handle9, -1, coord, sim.simx_opmode_blocking);
                    shepherds(3,:) = xycoord;
                end
            end
            ...
            for acciones=1:cnt 
                ...
                xycoord = SearchIndexOnTable(Q_board,accion);
                coord = [xycoord boidAlt];
                [res14] = sim.simxSetObjectPosition(clientID, target_handle14, -1, coord, sim.simx_opmode_blocking);
            end
            ...
            if cnt > 1   
                for acciones=1:2 
                    ...
                    xycoord = SearchIndexOnTable(Q_board,accion);
                    coord = [xycoord boidAlt];
                    [res10] = sim.simxSetObjectPosition(clientID, target_handle10, -1, coord, sim.simx_opmode_blocking);
                    shepherds(4,:) = xycoord;
                end
            end
