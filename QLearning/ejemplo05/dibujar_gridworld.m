...
function dibujar_gridworld(estado, num_filas, num_columnas, R,max_recompensa,pos)
    clf;
    grid on;
    ...
    [fila, columna] = ind2sub([num_filas, num_columnas], max_recompensa);
    plot(columna - 0.5, num_filas - fila + 0.5, 'go', 'MarkerSize', 10, 'MarkerFaceColor', 'g');

    [fila, columna] = ind2sub([num_filas, num_columnas], pos);
    plot(columna - 0.5, num_filas - fila + 0.5, 'bo', 'MarkerSize', 10, 'MarkerFaceColor', 'b');

   [fila, columna] = ind2sub([num_filas, num_columnas], estado);
    plot(columna - 0.5, num_filas - fila + 0.5, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
    
    legend('Recompensa','obstaculos','Agente','Location','NorthEastOutside','Orientation','vertical')
